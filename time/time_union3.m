function [X1u,X2u,T] = time_union3(X1,X2,FLAG,INTERVAL)
%TIME_UNION3 Put inputs on uniform time grid.  
%
%  X1u = TIME_UNION3(X1) Puts data on a sorted, uniform time grid spanning
%  the first interval to the last interval.  NaN is used as fill values.
%  X1 has columns of [YEAR, MONTH, DAY, DATA] and X1u has columns of DATA
%  only (with the same or greater number of rows as DATA).
%
%  [X1u,T] = TIME_UNION3(X1) returns T with columns of YEAR MONTH and DAY.
%
%  [X1u,T]     = TIME_UNION3(X1,INTERVAL) 
%  [X1u,X2u]   = TIME_UNION3(X1,X2,FLAG,INTERVAL) 
%  [X1u,X2u,T] = TIME_UNION3(X1,X2,FLAG,INTERVAL) 
%
...
for i = 1:length(Xc)
  low_year(i)   = Xc{i}(1,1);
  low_ymd(i,:)  = Xc{i}(1,1:3);
  high_year(i)  = Xc{i}(end,1);
  high_ymd(i,:) = Xc{i}(end,1:3);
...
Do    = 1;
Dno   = min(day_number(low_ymd));
[minyr,minmo,mindy] = ymd(Dno);
Df    = Npd*number_days(minyr,maxyr);
Dnf    = Npd*(max(day_number(high_ymd)));
...
for i = 1:length(Xc)
  ...
  if (M > a)
    ...
    Xu{i} = FLAG*ones(IX{i}(end)-IX{i}(1),M-a);
    Xu{i}(IX{i}-IX{i}(1)+1,:) = Xc{i}(:,a+1:M);
  else
    Xu{i} = [];
  end  
...
if (nargout == 3 & ~iscell(X1))
  if (INTERVAL(1:3) == 'day')
    T = ymd([1:(max(IX{1})-min(IX{1})+1)]',minyr,INTERVAL);
  else
    T = ymd([min(IX{1})-1:max(IX{1})-1]',minyr,INTERVAL);
  end
end

if (nargout == 2 & (iscell(X1) | length(Xu)==1))
  if (INTERVAL(1:3) == 'day')
    X2u = ymd([1:(max(IX{1})-min(IX{1})+1)]',minyr,INTERVAL);
  else
    X2u = ymd([min(IX{1})-1:max(IX{1})-1]',0,INTERVAL);
  end
end