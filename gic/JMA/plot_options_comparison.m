...
for i = 1:2
    fn=fn+1;figure(fn);clf;
    semilogx(T,sf*real(Opts{j}.GE_avg.Mean.Z(2:end,i)),'r',...
          'LineStyle','-','LineWidth',1);%'Marker','.','MarkerSize',20);
    hold on;grid on;axis tight;
    semilogx(T,sf*imag(Opts{j}.GE_avg.Mean.Z(2:end,i)),'b',...
          'LineStyle','--','LineWidth',1);%'Marker','.','MarkerSize',20);
    semilogx(T,sf*real(Opts{j}.GE_avg.Median.Z(2:end,i)),'g',...
          'LineStyle','-','LineWidth',1);%'Marker','.','MarkerSize',20);
    semilogx(T,sf*imag(Opts{j}.GE_avg.Median.Z(2:end,i)),'g',...
          'LineStyle','--','LineWidth',1);%'Marker','.','MarkerSize',20);
    semilogx(T,sf*real(Opts{j}.GE_avg.Huber.Z(2:end,i)),'b',...
          'LineStyle','-','LineWidth',1);%'Marker','.','MarkerSize',20);
    semilogx(T,sf*imag(Opts{j}.GE_avg.Huber.Z(2:end,i)),'b',...
          'LineStyle','--','LineWidth',1);%'Marker','.','MarkerSize',20);      
%    errorbars(T,...
%              sf*Opts{1}.GE_avg.Mean.Zabs(2:end,i),...
%              2*sf*Opts{1}.GE_avg.Mean.Zabs_StdErr(2:end,i),...
%              2*sf*Opts{1}.GE_avg.Mean.Zabs_StdErr(2:end,i),'y','g');
    title(sprintf('Options %d | p = %s',j,titles{i}));
    legend('Re(mean(p))','Im(mean(p))','Re(median(p))','Im(median(p)','Re(huber(p))','Im(huber(p))','Location','Best');
end
...
for i = 1:2
    ...
    title(sprintf('Options %d | %s',j,titles{i}));
