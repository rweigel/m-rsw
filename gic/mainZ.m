...
if strmatch('MBB05',short,'exact')
    ...
    % (PE < 0) and inconsistencies with my calculation of Z (Zxx and Zyy are reversed)    
    ZA = -fliplr(ZA);
    ...
    % EpA has units of [mV/km]/[nT] * [T]
    % Convert to V/m, units of measurement of E.
    EpA(:,1) = 1e3*EpA(:,1);
    EpA(:,2) = 1e3*EpA(:,2);
...
figure(fn);
    ...
    loglog(1./feR(2:end),10^(-3)*2*pi*feR(2:end).*abs(ZR(2:end,2)),'b','LineWidth',2);
    loglog(1./feR(2:end),10^(-3)*2*pi*feR(2:end).*abs(ZR(2:end,3)),'k','LineWidth',2);
    loglog(1./feR(2:end),10^(-3)*2*pi*feR(2:end).*abs(ZR(2:end,4)),'r','LineWidth',2);

    xlabel('T [s]')
    ylabel('[mV/km/nT]')
    legend('Z_0 USGS CP-1','Z_0 USGS PT-1',...
...
for i = 1:2
    ...
    figure(fn);clf;
        ...
        hold on;grid on;
        plot(t,(E(:,i)+Ef(:,i))*1e6,'m'); % Measured
        plot(t,E(:,i)*1e6,'k') % Filtered
        xlabel(xlab)
        ylabel('[mV/km]')
        legend(['E_',comp,' measured'],['E_',comp,' filtered'])
